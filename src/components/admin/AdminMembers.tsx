...
export const AdminMembers: React.FC = () => {
  ...
  const activePasswordChangedCount = members.filter(m => m.credentialStatus === 'Active' && !m.mustChangePassword).length;
  ...
  const handleAutoGenerateCredentials = () => {
    ...
    addToast('Generated new username & temporary password.', 'info');
  };
  ...
  const handleCopyCredentials = (m: Member) => {
    const username = m.username || 'Not assigned yet';
    const message = `🇵🇭 PAGASA GUIMBA YOUTH MIS - MEMBER PORTAL CREDENTIALS\n\n` +
      `Mabuhay, ${m.fullName}!\n` +
      `An administrator has assigned your official credentials for the PAGASA Guimba Member Portal.\n\n` +
      `🔗 Portal URL: ${window.location.origin}\n` +
      `👤 Username: ${username}\n` +
      `📧 Gmail: ${m.email}\n` +
      `🔑 Password: (Temporary password provided by admin)\n` +
      `📍 Barangay: Brgy. ${m.barangay}\n\n` +
      `Please sign in at ${window.location.origin} and set your private permanent password.`;
    
    if (navigator.clipboard) {
      navigator.clipboard.writeText(message);
      addToast(`Copied portal invitation & credentials message for ${m.fullName}!`, 'success');
    }
  };
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 space-y-4 my-6">
            ...
            <form onSubmit={handleSaveAssignedCredentials} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-bold text-slate-700">
                    Assigned Temporary Password <span className="text-red-500">*</span>
                  </label>
                  <button
                    type="button"
                    onClick={() => setAssignPassword(generateTemporaryPassword())}
                    className="text-[11px] font-bold text-blue-600 hover:text-blue-800 flex items-center gap-1 cursor-pointer"
                  >
              ...
              <div className="p-3 bg-blue-50/70 border border-blue-200/80 rounded-2xl space-y-2 text-xs">
                ...
                <label className="flex items-start gap-2 text-blue-950 font-medium cursor-pointer">
                  ...
                  />
                  <span>
                    <strong>Force password change</strong> upon first login (Recommended for security)
                  </span>
                </label>
              </div>
